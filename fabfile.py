...
env.install_path = '/opt/deployable_sql'
env.current = '/opt/deployable_sql/current'
env.repo = 'git@git.usccreditunion.org:cu/deployable_sql.git'
env.build = datetime.now().strftime('%Y%m%d%H%M%S')
env.settings = 'settings.p'
...
def deploy():
    ...
    with cd('%(install_path)s' % env):
        ...
        with cd('%(build)s' % env), prefix('workon sql'):
            run('pip install -U pip')
            run('pip install -r requirements.txt')
            run('nosetests -v')
            # if tests pass, change symlink
            remove = 'rm %(current)s' % env
...
def create():
    ...
    sudo('mkdir %(install_path)s' % env)
    sudo('chown -R %(user)s:%(user)s %(install_path)s' % env)
    with cd('%(install_path)s' % env):
        run('mkdir current')
        run('mkvirtualenv sql')
        #run('mv ~/%(settings)s %(current)s/%(settings)s' % env)
    # set up log dir
    sudo('mkdir /var/log/deployable_sql')
    sudo('chown -R %(user)s:%(user)s /var/log/deployable_sql' % env)
    deploy()

def oneoff():
    """Run deployable_sql as a one off process."""
    with cd('%(current)s' % env), prefix('workon sql'):
